...
- 使用markdown格式化标题，保持段落结构清晰

请始终使用清晰的段落结构输出，使用markdown格式化标题。"""
...
Respond ONLY with valid JSON."""
    ...
    def get_executive_summary_prompt(
        ...
        prompt = f"""请基于以下VC/PE市场分析数据，生成一份专业的执行摘要（约1500-2000字中文）。
...
请使用专业的VC/PE行业术语，保持客观中立的语调，用中文撰写所有内容。确保内容详实、数据准确、分析深入。目标字数1500-2000字。"""
...
"""
        ...
        prompt = f"""请基于以下VC/PE市场数据，生成一份完整的行业周报文章（要求2000字以上）。
